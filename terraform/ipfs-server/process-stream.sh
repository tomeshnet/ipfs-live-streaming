...
while true; do
  ...
  if ! [ -z "${nextfile}" ]; then
    ...
    resetStream=$(cat ~/streamReset)
    resetStreamMarker=''
    if [ "$resetStream" == '1' ]; then
      resetStreamMarker=" #EXT-X-DISCONTINUITY"
    fi
    ...
    if [[ ${ret} -eq 0 ]]; then
      ...
      echo $(cat ~/tmp.txt) ${time}.ts ${timecode}${resetStreamMarker} >> ~/process-stream.log
